...
function cot_add_user($ruser, $email = null, $name = null, $password = null, $maingrp = null, $sendemail = true)
{
	...
	if ($ruser['user_maingrp'] == 2 && $sendemail)
	{
		...
		else
		{
			...
			$cargo=$ruser['user_usergroup']==7?'m=regcargo':'m=regtransp';
			$activate = cot::$cfg['mainurl'].'/'.cot_url('users', $cargo.'&a=validate&token='.$ruser['user_token'].'&v='.$ruser['user_lostpass'].'&y=1', '', true);
			$deactivate = cot::$cfg['mainurl'].'/'.cot_url('users', $cargo.'&a=validate&token='.$ruser['user_token'].'&v='.$ruser['user_lostpass'].'&y=0', '', true);
