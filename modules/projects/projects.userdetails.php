...
foreach ($sql_projects_count_state as $value)
{
    ...
    $t1->assign(array(
        ...
        "PRJ_STAT_ROW_URL" => cot_url('users', 'm=details&id=' . $urr['user_id'] . '&u=' . $urr['user_name'] . '&tab=projects&stat='.$value['item_state']),
        "PRJ_STAT_ROW_TITLE"=>$L['projects_status_'.cot_projects_status($value['item_state'])],
        "PRJ_STAT_ROW_COUNT_PROJECTS"=>$value['state_count'],
        "PRJ_STAT_ROW_SELECT" => (($state||$state===0)&& $state == $value['item_state']) ? 1 : '',
    ));
    $t1->parse("MAIN.STAT_ROW");
}
/*foreach ($sql_projects_count_state as $value) {
