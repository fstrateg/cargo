    ...
    {session_start();
        if (isset($_POST['hot'])) {
            ...
            foreach ($_POST['hot'] as $key => $value) {
                ...
                $res = $db->query("UPDATE $db_spisok_transport SET hot='$hot' WHERE id='$id'");
            }
        }
    }
    ...
    {session_start();
        if (isset($_POST['del'])) {
            ...
            foreach ($_POST['del'] as $key => $value) {
                ...
                cot::$db->registerTable('spisok_transport');
                global $db, $db_spisok_transport;
                $res = $db->query("DELETE FROM $db_spisok_transport WHERE id='$id'");
...
if (isset($_POST['add_spisok'])) {
    {session_start();
        ...
        if (isset($_POST['add_name']))
        {
            ...
            cot::$db->registerTable('spisok_transport');
            global $db, $db_spisok_transport;
            $res = $db->query("SELECT name FROM $db_spisok_transport WHERE name = '$name'")->fetch();
            if (!$res){
            $hot = $_POST['add_hot'];
            $url = $_SERVER['HTTP_REFERER'];
