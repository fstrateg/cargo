...
function lz_login_translate($login){

	global $cot_translit, $cot_translit_custom;
	...
	if(file_exists(cot_langfile('translit', 'core')))
	{
		...
		{
			$login = strtr($login, $cot_translit_custom);
		}
		elseif (is_array($cot_translit))
		{
			$login = strtr($login, $cot_translit);
		}
	}else{
		cot_log('translit core error', 'adm');
	}
	$login = preg_replace('#[^\p{L}0-9\-_ ]#u', '', $login);
