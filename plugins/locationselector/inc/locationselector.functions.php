...
function cot_select_locationto($country = '', $region = 0, $city = 0, $userdefault = false)
{
	$result=cot_select_location($country, $region, $city, $userdefault);
	$result=str_replace('"locselectcountry"','"locselectcountryto"',$result);
	$result=str_replace('"locselectregion"','"locselectregionto"',$result);
	$result=str_replace('"locselectcity"','"locselectcityto"',$result);
	
	$result=str_replace('"country"','"countryto"',$result);
	$result=str_replace('"region"','"regionto"',$result);
	$result=str_replace('"city"','"cityto"',$result);
	return $result;
}
/**
...
function cot_import_location($source = 'P')
{
	...
	$result['countryto'] = cot_import('countryto',$source, 'ALP', 2);
	$result['regionto'] = cot_import('regionto', $source, 'INT');
	$result['cityto'] = cot_import('cityto', $source, 'INT');
	$result['regionto'] = ($result['countryto'] == '0') ? 0 : $result['regionto'];
	$result['cityto'] = ($result['regionto'] == 0) ? 0 : $result['cityto'];
