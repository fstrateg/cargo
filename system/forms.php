...
function cot_selectbox_date($utime, $mode = 'long', $name = '', $max_year = 2030, $min_year = 2000, $usertimezone = true, $custom_rc = '')
{
	...
	$year = '<span>'.cot_selectbox($s_year, $name.'[year]', range($max_year, $min_year, -1)).'</span>';
	$month = '<span>'.cot_selectbox($s_month, $name.'[month]', array_keys($months), array_values($months)).'</span>';
	$day = '<span>'.cot_selectbox($s_day, $name.'[day]', range(1, 31)).'</span>';
	...
	$result = '<span class="datesel">'.cot_rc($rc, array(
		'day' => $day,
		'month' => $month,
		'year' => $year,
		'hour' => $hour,
